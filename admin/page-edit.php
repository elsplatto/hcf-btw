...
{
    $mysqli = new mysqli($DB_SERVER, $DB_USERNAME, $DB_PASSWORD, $DB_DATABASE);
    $stmt = $mysqli->prepare('SELECT tiles.id, tiles.title, tiles.image_thumb FROM tiles WHERE tiles.is_live = 1 AND tiles.type_id = 1');
    $stmt->execute();
...
{
    $mysqli = new mysqli($DB_SERVER, $DB_USERNAME, $DB_PASSWORD, $DB_DATABASE);
    $stmt = $mysqli->prepare('SELECT tiles.id, tiles.title, tiles.image_thumb FROM tiles WHERE tiles.is_live = 1');
    $stmt->execute();
...
{
    $mysqli = new mysqli($DB_SERVER, $DB_USERNAME, $DB_PASSWORD, $DB_DATABASE);
    $stmt = $mysqli->prepare('SELECT page_id, tile_id, page_tile.order FROM page_tile WHERE page_id = ? ORDER BY page_tile.order');
    $stmt->bind_param('i', $id);
    $stmt->execute();
...
<section>
    <form id="frmPage" name="frmPage" action="page-process.php" method="post">
        ...
        foreach ($pages as $page)
        {
                ...
                <ul class="tileList">
                    ...
                    foreach ($allPageTiles as $pageTile)
                    {
                        ...
                        foreach ($selectedPageTiles as $selectedPageTile)
                        {
                            ...
                            {
                                $strSelected = ' data-page-tile-selected';
                                break;
                            }
                        }
                        ?>
                        <li<?=$strSelected?> data-tile-id="<?=$pageTile['id']?>" data-page-id="<?=$page_id?>"><?=$pageTile['id']?><img src="../img/locations/thumbnails/<?=$pageTile['image_thumb']?>" /></li>
                    <?php
                    }
                    ?>
                </ul>
            ...
            <input type="checkbox" id="chkLive" name="chkLive" value="1"<?php echo ($page['is_live']?'checked="checked"':'')?> />
